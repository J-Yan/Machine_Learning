...
class KNN:
    ...
    def predict(self, features):
        ...
        for x in range(len(features)):
            # print(Counter(self.get_k_neighbors(features[x])).most_common(1)[0][0])
            predList.append(Counter(self.get_k_neighbors(features[x])).most_common(1)[0][0])
    ...
    def get_k_neighbors(self, point):
        ...
        for x in range(len(self.features)):
            dist = self.distance_function(point, self.features[x])
            distances.append((self.labels[x],dist))
